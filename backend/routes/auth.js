        ...
        try {
            ...
            if (req.body.skillSets) {
                try {
                    ...
                    console.error('Skillsets parsing error in auth simple register:', e);
                }
            }
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            // Parse skillSets if it's a string (FormData sends arrays as strings or multiple fields)
            let parsedSkillSets = {
                technical: [],
                creative: [],
                cognitive: [],
            ...
            if (skillSets) {
                try {
                    parsedSkillSets = typeof skillSets === 'string' ? JSON.parse(skillSets) : skillSets;
                } catch (e) {
                    console.warn('Failed to parse skillSets, using default empty sets');
                }
            }
            ...
            if (skillSets) {
                try {
                    skillSetsParsed = typeof skillSets === 'string' ? JSON.parse(skillSets) : skillSets;
                } catch (e) {
                    console.warn('Failed to parse skillSets in auth register-enhanced, using default empty sets');
                }
            }
...
router.post('/login', async (req, res) => {
    try {
        ...
        if (adminCount === 0) {
            ...
            await user.save();
            console.log('Bootstrapping login user to admin (auth):', user.email);
        }
