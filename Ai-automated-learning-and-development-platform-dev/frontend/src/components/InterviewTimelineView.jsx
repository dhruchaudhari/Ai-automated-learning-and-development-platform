...
const InterviewTimelineView = ({ filteredUsers, panels, onViewUser, searchTerm }) => {
    ...
    }, [flattenedData, searchTerm, filterDays, filterRange, filterWeeks, viewPreset]);
    ...
    const { lineData, barData, dateUserMap, stats, dateRange } = useMemo(() => {
        const scheduledUsers = filteredCandidates.filter(u => u.resolvedSchedule);
        const emailSentUsers = filteredCandidates.filter(u => u.resolvedEmailSent);

        const st = {
            scheduled: scheduledUsers.length,
            emailSent: emailSentUsers.length,
            panelAssigned: filteredCandidates.reduce((sum, u) => {
        ...
        const dates = scheduledUsers.map(u => new Date(u.resolvedSchedule)).filter(d => isValid(d));
        if (dates.length === 0) {
            return { lineData: [], barData: [], dateUserMap: {}, stats: st, dateRange: null };
        }
        ...
        if (viewPreset === "daywise") {
            ...
            scheduledUsers.forEach(u => {
                const d = getDay(new Date(u.resolvedSchedule));
                scheduledGrouped[d]++;
                userMap[d].push(u);
                if (u.resolvedEmailSent) emailSentGrouped[d]++;
            });
            xLabels = days.map((label, i) => ({ label, key: i }));

        } else if (viewPreset === "monthwise") {
            scheduledUsers.forEach(u => {
                const k = format(new Date(u.resolvedSchedule), "MMM yyyy");
                ...
                scheduledGrouped[k]++;
                userMap[k].push(u);
                if (u.resolvedEmailSent) emailSentGrouped[k]++;
            });
            ...
            scheduledUsers.forEach(candidate => {
                const key = format(new Date(candidate.resolvedSchedule), "yyyy-MM-dd");
                if (scheduledGrouped[key] !== undefined) {
                    scheduledGrouped[key]++;
                    userMap[key].push(candidate);
                }
                if (candidate.resolvedEmailSent && emailSentGrouped[key] !== undefined) {
        ...
        return Object.entries(dateUserMap)
            ...
            .map(([key, users]) => {
                ...
                return {
                    ...
                    dateLabel: label,
                    users,
                    emailSent: users.filter(u => u.resolvedEmailSent).length,
                    panelAssigned: users.filter(u => {
                        if (!u.currentAd) return u.panelAssignments?.length > 0;
                        const adId = (u.currentAd._id || u.currentAd).toString();
                        return u.panelAssignments?.some(pa => pa.advertisementId?.toString() === adId);
                    }).length
                };
            });
        ...
        return (
            <div className="bg-white rounded-xl shadow-2xl border border-indigo-100 p-4 max-w-[300px] pointer-events-none" style={{ zIndex: 99999 }}>
                <div className="flex items-center gap-2 mb-2 pb-2 border-b border-gray-100">
                    ...
                    <span className="text-sm font-medium text-gray-800">{point?.data?.x}</span>
                    <span className={`ml-auto text-xs font-medium px-2 py-0.5 rounded-full ${isEmailSeried ? "bg-emerald-50 text-emerald-700" : "bg-indigo-50 text-indigo-700"
                        }`}>
                ...
                {usersOnDate.length > 0 && !isEmailSeried && (
                    <div className="space-y-1.5">
                        {usersOnDate.slice(0, 5).map(candidate => {
                            const pNames = getPanelNames(candidate);
                            return (
                                <div key={candidate.uniqueId} className="flex flex-col p-1.5 rounded-lg bg-gray-50 border border-gray-100">
                                    <div className="flex items-center gap-2">
                                        {candidate.profileImage ? (
                                            <img src={candidate.profileImage} alt="" className="w-5 h-5 rounded-full object-cover border border-white shadow-sm flex-shrink-0" />
                                        ) : (
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <p className="text-[11px] font-medium text-gray-700 truncate">{candidate.fullName}</p>
                                            <p className="text-[8px] text-indigo-500 font-bold truncate">{candidate.currentAd?.title || "N/A"}</p>
                                        </div>
                                        {candidate.resolvedEmailSent ? (
                                    ...
                                    {pNames.length > 0 && (
                                        <div className="mt-1 flex flex-wrap gap-1 px-1">
                                            {pNames.map((name, i) => (
                                                <span key={i} className="text-[7px] text-purple-500 font-medium px-1 bg-purple-50 rounded border border-purple-100 whitespace-nowrap">{name}</span>
                                            ))}
                                        </div>
                                    )}
                                </div>
        ...
        return (
            <div className="bg-white rounded-xl shadow-2xl border border-indigo-100 p-3 max-w-[280px] pointer-events-none">
                <div className="flex items-center gap-2 mb-2 pb-1.5 border-b border-gray-100">
                    <FaCalendarAlt className="text-indigo-500 text-xs" />
                    <span className="text-sm font-medium text-gray-800">{indexValue}</span>
                    <span className={`ml-auto text-xs font-medium px-2 py-0.5 rounded-full ${id === "Emails Sent" ? "bg-emerald-50 text-emerald-700" : "bg-indigo-50 text-indigo-700"
                        }`}>
                        {value} {id}
                    </span>
                </div>
                {usersOnGroup.length > 0 && id === "Interviews" && (
                    <div className="space-y-1">
                        {usersOnGroup.slice(0, 4).map(candidate => (
                            <div key={candidate.uniqueId} className="flex items-center gap-1.5 text-[11px]">
                                <FaUser className="text-[8px] text-indigo-400 flex-shrink-0" />
                                <span className="font-medium text-gray-700 truncate">{candidate.fullName}</span>
                                <span className="text-[8px] text-indigo-500 truncate max-w-[60px]">({candidate.currentAd?.title || "N/A"})</span>
                                {candidate.resolvedEmailSent && <FaCheckCircle className="text-[8px] text-emerald-500 flex-shrink-0" />}
                            </div>
                        ))}
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-gradient-to-br from-indigo-50 to-purple-50 rounded-2xl border border-indigo-100">
                    ...
                    {(viewPreset !== "datewise" || searchTerm || filterDays.length > 0 || filterRange.start || filterRange.end || filterWeeks.length > 0) && (
                        <button
                            ...
                            className="flex items-center gap-2 px-6 py-2.5 bg-indigo-600 text-white rounded-xl font-medium text-sm shadow-lg hover:shadow-indigo-200 hover:bg-indigo-700 transition-all cursor-pointer"
                        >
                    ...
                    <div className="flex items-center gap-3 flex-1 max-w-2xl">
                        ...
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="w-full pl-10 pr-10 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none shadow-sm text-gray-400 italic"
                            />
                            {searchTerm && (
                        ...
                        <div className="relative">
                            <select
                                ...
                                className={`appearance-none flex items-center gap-2 pl-9 pr-8 py-2.5 rounded-xl border font-medium text-xs transition-all shadow-sm cursor-pointer outline-none ${viewPreset !== "datewise"
                        ...
                        {(filterDays.length > 0 || filterRange.start || filterRange.end || filterWeeks.length > 0) && (
                            <button
                                ...
                                className="flex items-center gap-2 px-3 py-2.5 rounded-xl bg-purple-50 border border-purple-200 text-purple-700 font-medium text-xs shadow-sm hover:bg-purple-100 transition-all cursor-pointer"
                            >
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <div className="flex items-center bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                            {CHART_TYPES.map(({ key, label, icon: Icon }) => (
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 px-3.5 py-2 text-xs font-medium transition-all cursor-pointer ${chartType === key
                        ...
                        {chartType !== "bar" && (
                            <button
                                ...
                                className={`flex items-center gap-1.5 px-4 py-2 rounded-xl border text-xs font-medium transition-all cursor-pointer shadow-sm ${showCumulative
                                    ? "bg-indigo-600 text-white border-indigo-600"
                                    : "bg-white text-gray-600 border-gray-200 hover:bg-indigo-50"
                                    }`}
                            >
                ...
                {[
                    ...
                    <div key={label} className={`p-3 rounded-xl bg-gradient-to-br ${bg} border ${border} shadow-sm`}>
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                <p className="text-[9px] text-gray-500 font-medium tracking-wider uppercase">{label}</p>
                                                    ...
                                                    >
                                                        ...
                                                        <div className="min-w-0 flex-1">
                                                            ...
                                                            {user.currentAd?.title && (
                                                                <p className="text-[10px] text-indigo-600 font-bold uppercase tracking-tight bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-100 inline-block mb-1">
                                                                    {user.currentAd.title}
                                                                </p>
                                                            )}
