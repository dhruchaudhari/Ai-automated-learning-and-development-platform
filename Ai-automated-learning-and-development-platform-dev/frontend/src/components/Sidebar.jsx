...
const Sidebar = () => {
  ...
  const { logout, isAdmin, isDeptHead } = useAuth();
  ...
  const adminMenuItems = [
    ...
    {
      name: 'Fitness & Allocation Optimization Intelligence',
  ...
  // Dept Head menu items
  const deptHeadMenuItems = [
    {
      name: 'Dept Head Control',
      path: '/dept-head-control',
      icon: <UsersIcon className="w-5 h-5" />,
    },
  ];

  // Combine menu items based on role
  const menuItems = isAdmin ? adminMenuItems : isDeptHead ? deptHeadMenuItems : userMenuItems;
        ...
        {userData && (
          <div className="mt-3 pt-3 border-t border-primary-700/20">
            <p className="text-xs text-primary-400/70 text-center truncate">
              {`Signed in as ${userData.username || userData.fullName || "User"}`}
            </p>
          </div>
        )}
      </div>
