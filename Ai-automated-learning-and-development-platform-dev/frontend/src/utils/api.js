...
export const normalizationAPI = {
  ...
  getFinalMerits: () => api.get('/normalization/final-merits'),
  getFinalMeritByAd: (advertisementId) => api.get(`/normalization/final-merit/${advertisementId}`),
  runFitness: (advertisementId, meritListId) => api.post('/normalization/run-fitness', { advertisementId, meritListId }),
  saveFitnessAllocation: (data) => api.post('/normalization/save-fitness-allocation', data),
  saveFitnessAllocationBatch: (batchItems) => api.post('/normalization/save-fitness-allocation-batch', { batchItems })
};

// ==================== DEPT HEAD API CALLS ====================
export const deptHeadAPI = {
  login: (email, password) => api.post('/deptheads/login', { email, password }),
  signup: (data) => api.post('/deptheads/signup', data)
};
