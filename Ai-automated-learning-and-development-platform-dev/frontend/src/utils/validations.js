...
export const validateAdTitle = (title) => {
  if (!title || title.trim().length === 0) return 'Advertisement title is required';
  if (title.trim().length < 5) return 'Title must be at least 5 characters';
  if (title.trim().length > 200) return 'Title is too long';
  return '';
};

export const validateLastDate = (date) => {
  if (!date) return 'Last date to apply is required';
  const inputDate = new Date(date);
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  if (inputDate < today) return 'Last date cannot be in the past';
  return '';
};

export const validateAdPDF = (file, isEdit = false) => {
  if (!file && !isEdit) return 'Advertisement PDF is required';
  if (!file && isEdit) return '';

  if (typeof file === 'string' && file.startsWith('/uploads')) return '';

  if (!(file instanceof File)) return 'Invalid file format';
  if (file.type !== 'application/pdf') return 'Only PDF files are allowed';
  if (file.size > 10 * 1024 * 1024) return 'PDF must be less than 10MB';
  return '';
};
